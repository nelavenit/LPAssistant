...
test("print pivot rings stay cell-sized and printed tableaux are contiguous", async () => {
  ...
  assert.match(printCss, /\.history-card \.tableau-scroll \{ border-left: 1px solid #777; border-right: 1px solid #777; border-top: 0; border-bottom: 0; \}/);
...
test("0.7.0 typography, hover continuity, exports, and LPAssistant identity are wired", async () => {
  ...
  assert.match(css, /\.history-card \.tableau-scroll \{ border-left: 1px solid #777; border-right: 1px solid #777; border-top: 0; border-bottom: 0; \}/);
  assert.match(css, /\.history-card:first-child \.tableau-scroll, \.history-card \+ \.history-card \.tableau-scroll \{ border-top: 1px solid #777; \}/);
  assert.match(css, /\.history-card:last-child \.tableau-scroll \{ border-bottom: 1px solid #777; \}/);
  assert.match(css, /\.tableau-grid \{ width: 100%; min-width: 0 !important; \}/);
  ...
  assert.match(app, /className="control-tooltip" role="tooltip"/);
